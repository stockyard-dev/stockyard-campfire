...
func Open(d string) (*DB, error) {
	...
	db, err := sql.Open("sqlite", filepath.Join(d, "campfire.db")+"?_journal_mode=WAL&_busy_timeout=5000")
	if err != nil {
		return nil, err
	}
...
func (d *DB) CreateChannel(c *Channel) error {
	c.ID = genID()
	c.CreatedAt = now()
	_, err := d.db.Exec(`INSERT INTO channels(id,name,description,created_at)VALUES(?,?,?,?)`, c.ID, c.Name, c.Description, c.CreatedAt)
	return err
}
func (d *DB) ListChannels() []Channel {
	rows, _ := d.db.Query(`SELECT id,name,description,created_at FROM channels ORDER BY name`)
	if rows == nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		d.db.QueryRow(`SELECT COUNT(*) FROM messages WHERE channel_id=?`, c.ID).Scan(&c.MessageCount)
		o = append(o, c)
	}
	return o
}
func (d *DB) DeleteChannel(id string) error {
	d.db.Exec(`DELETE FROM messages WHERE channel_id=?`, id)
	_, err := d.db.Exec(`DELETE FROM channels WHERE id=?`, id)
	return err
}
func (d *DB) PostMessage(m *Message) error {
	m.ID = genID()
	m.CreatedAt = now()
	if m.Author == "" {
		m.Author = "anonymous"
	}
	_, err := d.db.Exec(`INSERT INTO messages(id,channel_id,author,body,created_at)VALUES(?,?,?,?,?)`, m.ID, m.ChannelID, m.Author, m.Body, m.CreatedAt)
	return err
}
func (d *DB) ListMessages(channelID string, limit int) []Message {
	if limit <= 0 {
		limit = 100
	}
	rows, _ := d.db.Query(`SELECT id,channel_id,author,body,created_at FROM messages WHERE channel_id=? ORDER BY created_at DESC LIMIT ?`, channelID, limit)
	if rows == nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		var m Message
		rows.Scan(&m.ID, &m.ChannelID, &m.Author, &m.Body, &m.CreatedAt)
		o = append(o, m)
	}
	return o
}
...
func (d *DB) Stats() map[string]any {
	var ch, msg int
	d.db.QueryRow(`SELECT COUNT(*) FROM channels`).Scan(&ch)
	d.db.QueryRow(`SELECT COUNT(*) FROM messages`).Scan(&msg)
	return map[string]any{"channels": ch, "messages": msg}
}
